...
pub trait Client : Send + Sync {
    ...
    fn poll_batches_raw(&self, topic: String, partition: u32, consumer_group: String, offset: u64, poll_config: PollConfig) -> Result<PollBatchesRawResponse>;
}
...
impl Client for ClientImpl {
    ...
    fn poll_batches_raw(&self, topic: String, partition: u32, consumer_group: String, offset: u64, poll_config: PollConfig) -> Result<PollBatchesRawResponse> {
        let response = self.api_client.post(POLL_BATCHES_RAW, PollBatchesRequest { topic, partition, consumer_group, offset, poll_config })?;
