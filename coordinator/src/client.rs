...
use crate::client::Error::InvalidReadOffset;
...
impl Client {
    ...
    /// # Errors
    ///
    /// - Returns [`Error::TopicAlreadyExists`] if the topic already exists.
    /// - Returns [`Error::Internal`] for any other database error.
    pub fn create_topic(&self, topic: &str, partition_count: u32) -> Result<()> {
        let result = self.connection.execute("INSERT INTO topics (name, partition_count) VALUES (:topic, :partition_count)", &[
            (":topic", topic),
        ...
        match result {
            ...
            Err(e) => Err(match e {
                SqliteFailure(cause, _) if cause.code == ConstraintViolation => {
                    Error::TopicAlreadyExists(topic.to_string())
                }
                _ => Error::Internal(e),
            })
        }
    ...
    /// # Errors
    ///
    /// - Returns [`Error::TopicNotFound`] if the topic does not exist.
    /// - Returns [`Error::Internal`] for any other database error.
    pub fn get_topic(&self, name: &str) -> Result<Topic> {
        ...
        .map_err(|e| match e {
            rusqlite::Error::QueryReturnedNoRows => Error::TopicNotFound(name.to_string()),
            _ => Error::Internal(e),
        })
    }
    ...
    ///
    /// - Returns [`Error::TopicNotFound`] if the topic does not exist.
    /// - Returns [`Error::OutOfRangePartition`] if the requested partition is out of range.
    /// - Returns [`Error::Internal`] for any other database error.
    ...
    pub fn get_write_offset(&self, topic_name: &str, partition: u32) -> Result<Option<u64>> {
        ...
        Self::to_opt_offset(self.connection.query_one(
    ...
    /// - Returns [`Error::TopicNotFound`] if the topic does not exist.
    /// - Returns [`Error::OutOfRangePartition`] if the requested partition is out of range.
    /// - Returns [`Error::InvalidReadOffset`] if the requested offset exceeds the write offset for this partition, or no data has been written yet
    /// - Returns [`Error::Internal`] for any other database error.
    ...
    fn validate_topic_and_partition(&self, topic_name: &str, partition: u32) -> Result<()> {
        ...
        if partition >= topic.partition_count {
            Err(Error::OutOfRangePartition { topic, invalid_partition: partition })
        } else {
            Ok(())
        }
    }
